...
class Config:
    source_dir = 'data/preprocessed/single_1d_1024_no_IPv6'
    output_dir = 'data/split/single_1d_1024_no_IPv6'
    mode = '1d'  # [1d/2d]
    #max_len = 68594
    max_len = [5000, 5000, 5000, 5000, 2500]
    client_num = 2
    data_shape = 1024
...
def load_data_1d(config: Config, num_setting):
    ...
    for label, l_info in num_setting.items():
        ...
        val_num = 1000
        train_num = int(x_label.shape[0] - val_num*2)
...
def load_data_2d(config: Config, num_setting):
    ...
    for label, l_info in num_setting.items():
        ...
        print(x_label.shape[0])
        val_num = 900
        train_num = int(x_label.shape[0] - 2*val_num)
...
def getNumSetting(folder, max_len=[10800, 6300, 2800, 2300, 1960]):
    lab_count = 0
    counter = {}
    num_setting = {}
    ...
    for label, l_info in counter.items():
        l_max_len = l_info['cnt'] if l_info['cnt'] <= max_len[lab_count] else max_len[lab_count]
