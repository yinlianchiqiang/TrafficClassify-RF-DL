...
import models.dnn as model_dnn
...
class Config:
    ...
    output_dir = 'train_data/lab07E'

    model_file_name = 'fl_dnn_client'
...
def main():
    ...
    x_train, y_train, l = io_util.load_train_data_npz(dataset_train_fn, 42)
    x_test, y_test, l = io_util.load_train_data_npz(dataset_test_fn, 42)

    # train
    model = model_dnn.dnn(input_shape=Config.model_input_shape,
                          dropout=Config.model_dropout)

    # 啟動 client
    _client = TestClient(model_path, model, x_train, y_train, x_test, y_test, args.idx)
